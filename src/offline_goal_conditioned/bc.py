...
class BC(object):
    def __init__(self, env, args):
        ...
        self.expert_policy = GaussianPolicy(self.state_dim, self.action_dim, args.hidden_size, env.action_space).to(self.device)
        self.load_policy(self.expert_policy, args.expert_policy_dir)
        ...
        self.log_dir = f'runs/offline_goal_conditioned/{datetime_st}_BC_{args.env_name}'
        self.writer = SummaryWriter(self.log_dir)
        self.save_log_file(args)
    
    def load_policy(self, policy, model_dir):
    ...
    def evaluate(self, num=10, test=False):
        ...
        def rollout():
            ...
            if not test:
                ...
                state, action, goal, _, _, _ = self.memory.sample(1)
                state = state[0]
    ...
    def sample_expert_trajectory(self):
        states = []
        actions = []
        rewards = []
        next_states = []
        ...
        for t in range(self.traj_len):
            ...
            goals.append(goal)
            state = torch.FloatTensor(state).to(self.device)
            _, _, action = self.expert_policy.sample(state[None])
            action = action.detach().cpu().numpy()[0]

            actions.append(action)
            next_state, reward, done, _ = self.env.step(action[None])
            state = next_state
            rewards.append(reward)
    ...
    def train(self):
        ...
        while epoch < self.num_epochs:
            ...
            if epoch % self.sample_freq == 0:
                states, actions, rewards, next_states, goals, dones = self.sample_expert_trajectory()
